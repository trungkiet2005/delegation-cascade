...
statutory floor under attributed responsibility closes it and, over its
attainable range on the reported grid, closely traces the safety and welfare
frontier of a cap on chain length without anyone counting hand-offs.
\end{abstract}
...
every $0<\phi<1$ there is therefore a depth beyond which each further hand-off
strictly reduces the harm the principal is charged for while never reducing
...
game's long-run unsafe frequency falls below $10^{-3}$. At that setting, imperfect
transmission alone yields $0.018$ and imperfect attribution alone yields
$0.011$, while the two together yield $0.322$. The interaction,
$+0.294$, is $91\%$ of the joint effect. A counterfactual that holds the population composition fixed shows why. Under
...
frequency to $0.033$, but taking it to $0.04$ raises it again to $0.239$,
...
\begin{equation}
  ...
  \label{eq:wedge}
\end{equation}
The wedge~\eqref{eq:wedge} vanishes identically at $\phi=\lambda=1$, and it is
non-decreasing in $d$ whenever $\lambda\in[0,1]$, $\phi\in[0,1]$ and the
hand-off kernel satisfies the hypothesis of Proposition~\ref{prop:monotone}:
$\phi^{d}$ is then non-increasing, so $1-\lambda\phi^{d}$ is non-negative and
non-decreasing, $m$ is non-negative and non-decreasing in $d$ by that
proposition, and the product of two such factors is non-decreasing. Neither
restriction is vacuous, since the uniform kernel fails the hypothesis and the
robustness section sweeps $\phi$ above one.
Depth alone widens the gap between what a principal
...
$10^{-3}$ for every $L\ge 6.54$; the bisection threshold is $L_c=6.537432$.
We run at $L=20$, or $3.1$ times the level at which the single-layer race is
already safe. At that level the
...
\label{thm:shelter}
Fix $\phi>0$. Adding a hand-off lowers the attributed harm exactly when
\begin{equation}
  \phi \;<\; \phi^{*}(d) \;=\; \frac{m(d)}{m(d+1)} .
  \label{eq:phistar}
\end{equation}
Because $m$ is non-decreasing and bounded, $\phi^{*}(d)\to 1$. Hence for every
$0<\phi<1$ there exists a depth $\bar d(\phi)$ such that every hand-off beyond
$\bar d$ strictly reduces the harm attributed to the principal while never
reducing the harm it causes, and
\[
...
The excluded boundary $\phi=0$ is degenerate: attributed harm is zero at every
positive depth, so after the first hand-off it cannot fall strictly.
...
all $d$ large enough whenever $0<\phi<1$ -- the advantage is increasing in $L$,
...
\label{cor:failure}
Suppose $L\ge L_c>0$ and $0<\phi<1$. A liability $L$ that clears a threshold
$L_c$ for principals who act for themselves clears it at integer depth $d$
exactly when $d\le d^{\dagger}=\log(L_c/L)/\log\phi$; its first failing integer
depth is $\lfloor d^{\dagger}\rfloor+1$. At the baseline, $L_c=6.537432$,
$L=20$ and $\phi=0.75$ give $d^{\dagger}=3.887$: the rule survives three
hand-offs and fails at the fourth.
\end{corollary}
...
one because realised harm saturates, so any positive attribution retention below one is
...
$0.322$. The interaction, $+0.294$, is $91\%$ of the joint effect, and the same
...
payoff matrices rather than sampled. The index is positive over $91\%$ of the
interior and reaches $0.813$; the long-run unsafe frequency reaches $0.990$ and
...
$0.111$; by $\phi=0.99$ the outcome is the pass-through outcome. The location of
...
therefore $38\%$ overall, the ecosystem is still at $0.315$.
...
$\kappa=1$ down to $0.033$ at $\kappa=0.35$, and then reverses: at $\kappa=0.20$ the
...
The full-strength audit-placement sweep has the same kind of non-monotone
interior hump: unsafe frequency across layers one through six is $0.225$,
$0.136$, $0.064$, $0.071$, $0.157$ and $0.018$. At half strength, where the
intent margin does not flip, the sweep is monotone.
...
a weak version of pass-through; it closely approximates a depth cap over the
welfare range it can reach. Table~\ref{tab:floorcap} matches each ceiling to
the floor with the nearest social payoff. For ceilings two through six, whose
social payoffs lie within the floor's attainable range, the unsafe-frequency
gap is below $0.001$ (mean $0.0003$). Ceilings zero and one lie outside that
range. Across variations in $\phi$, $\varepsilon$ and the ceiling, the mean gap
ranges from $0.002$ to $0.016$; varying the organisational benefit expands the
range to $0.0003$--$0.0268$. The floor has no effect at all while it lies under
...
The interaction is positive in $91\%$ of them, with median $+0.294$ and
interquartile range $[0.243,0.299]$; the median interaction share is $0.91$.
...
all three population sizes and moves only from $0.3168$ to $0.3225$ as the
...
eliminates the effect: the joint cell falls to $2\times10^{-5}$ and mean depth
...
direction, makes matters worse rather than better, with a joint cell of $0.520$.
...
fifths of the range before rising to $0.520$ at pure noise, and mean depth sits
...
while the joint outcome ranges from $2\times10^{-5}$ to $0.520$. Directionality
...
of responsibility at each of six layers leaves the ecosystem at $0.315$ against
...
always comes back to the commissioning principal is a floor, and, over their
attainable range on the reported grid, floors closely trace the depth-cap
frontier. The legal analogue is familiar: where an injurer's assets
...
falling, and a rule that stops falling closes it. Over its attainable range on
the reported grid, a floor under attribution closely approximates the safety
and welfare outcomes of a cap on chain length, while asking less of whoever has
to enforce it. If there is a single design principle here, it is that the length of a
