...
\begin{table}[pos=t]
\centering
\caption{Attribution regimes. Long-run Unsafe frequency, mean delegation depth and social payoff when the law mapping depth to the principal's share of the harm is changed, with everything else at the baseline. The shelter survives every rule whose attribution vanishes in the depth, including the equal split. A rule bounded below by $a_{\min}$ closes it only above the depth $d_{0}$ at which the bound binds, $\phi^{d_{0}}=a_{\min}$, which is why $a_{\min}=0.25$ ($d_{0}=4.8$) still leaves mean depth at $5.07$ while $a_{\min}=0.5$ ($d_{0}=2.4$) brings it to $2.99$.}
\label{tab:regimes}
